...
from src.optimization.search.causal.interest_no_freq_search import InterestNoFreqSearch


class InterestNoFreqGen(AbstractBaseline):

    def __init__(self, env, bb_model, params=None):
        self.obj = BackObj(env, bb_model, params)
        self.optim = InterestNoFreqSearch(env, bb_model, self.obj, params)

        super(InterestNoFreqGen, self).__init__()
