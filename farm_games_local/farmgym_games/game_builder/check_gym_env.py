...
def _check_returned_values(env: gym.Env, observation_space: spaces.Space, action_space: spaces.Space) -> None:
    """
    Check the returned values by the gym_env when calling `.reset()` or `.step()` methods.
    """
    # because gym_env inherits from gym.Env,
    ...
    if render_modes is None:
        if warn:
            logger.warn(
                ...
                " (gym_env.metadata['render.modes'] is None or not defined), "
...
def check_gym_env(env: gym.Env, warn: bool = True, skip_render_check: bool = True) -> None:
    ...
    try:
        ...
        assert str(e) == "Cannot call gym_env.step() before calling reset()"
